    ...
    def __init__(self, cityscapes_data_path, cityscapes_meta_path):
        ...
        for val_dir in val_dirs:
            ...
            for file_name in file_names:
                ...
                example = {"img_path": img_path, "label_img_path": label_img_path, "img_id": img_id}
                self.examples.append(example)
...
class DatasetSeq(torch.utils.data.Dataset):
    def __init__(self, cityscapes_data_path):
        self.img_dir      = cityscapes_data_path + "/data/deeplabv3_panorama/"
        self.img_dir_half = cityscapes_data_path + "/data/lama"
        self.img_fisheye  = cityscapes_data_path + "/data/fisheye"
        ...
        for file_name in file_names:
            ...
            #----------------------------------------------------------------------------------------------#
            # the underlying surface of the original image is removed and converted into a fisheye image
            #----------------------------------------------------------------------------------------------#
            panorama = Image.open(img_path)
            width, height = panorama.size
            panorama = panorama.crop((0, 0, width, height / 2))
            if not os.path.exists(self.img_dir_half):
                os.makedirs(self.img_dir_half)
            panorama.save(os.path.join(self.img_dir_half, file_name))
            fisheye_name = file_name[:-4] + "_fisheye.png"
            BSVCalculation.panorama2fisheyeDeepLab(os.path.join(self.img_dir_half, file_name), 
                                                   os.path.join(self.img_fisheye, fisheye_name), False)
            example = {}
            example["img_path"] = img_path
